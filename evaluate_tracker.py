...
def load_mot_format(filepath):
    """ Load MOT format data into a dataframe """
    return np.loadtxt(filepath, delimiter=',')

def compute_mota(ground_truth_path, tracking_path):
    """ Compute MOTA score for given ground truth and tracking result paths """
    acc = mm.MOTAccumulator(auto_id=True)

    # Load ground truth and tracking data
    gt = load_mot_format(ground_truth_path)
    t = load_mot_format(tracking_path)
    
    # Iterate over frames in ground truth data
    ...
    summary = mh.compute(acc, metrics=['num_frames', 'idf1', 'idp', 'idr', \
                                       'recall', 'precision', 'num_objects', 'num_unique_objects', \
    ...
    strsummary = mm.io.render_summary(
        ...
        namemap={'idf1': 'IDF1', 'idp': 'IDP', 'idr': 'IDR', 'recall': 'Rcll', \
                 'precision': 'Prcn', 'num_objects': 'GT_Frame', 'num_unique_objects': 'GT_Trj',\
