"""OpenCV-based data augmentation pipeline for turkey ReID — v2.

Changes from augmentation.py
-----------------------------
1. p_rotation : 0.05 → 0.25   (turkeys face any direction; was rarely applied)
2. p_zoom     : 0.10 → 0.20   (scale variation from varying camera distance)
3. p_erasing  : 0.40 → 0.30   (40% was too aggressive for 3-8 images per ID)
4. New: HSV hue + saturation jitter (p_hsv=0.30) to simulate barn lighting
   variation that colour-space brightness+contrast jitter cannot capture.

All transforms operate on uint8 BGR images, consistent with the DeepSORT
pipeline that also works in BGR colour space.

Augmentations (in pipeline order):
  1. Random horizontal flip
  2. Random rotation           (p=0.25, ±15°)
  3. Random zoom               (p=0.20, 0.9–1.1×)
  4. Random crop-with-reflect-padding
  5. Brightness + contrast jitter  (always applied, small magnitude)
  6. HSV hue + saturation jitter   (p=0.30)   ← NEW
...
class ReIDAugmentation:
    ...
    p_rotation   : probability of random rotation  (v2: 0.25)
    max_rotation : maximum rotation angle in degrees
    p_zoom       : probability of random zoom      (v2: 0.20)
    zoom_range   : zoom range as (min_scale, max_scale)
    p_erasing    : probability of random erasing   (v2: 0.30)
    ...
    p_hsv        : probability of HSV hue+saturation jitter  (v2 new)
    hue_delta    : maximum hue shift in OpenCV H units (0–180)
    saturation   : maximum relative saturation multiplier deviation
    ...
    def __init__(
        ...
        p_flip: float = 0.5,
        p_rotation: float = 0.25,       # v2: was 0.05
        max_rotation: float = 15.0,
        p_zoom: float = 0.20,           # v2: was 0.10
        zoom_range: Tuple[float, float] = (0.9, 1.1),
        p_erasing: float = 0.30,        # v2: was 0.40
        p_blur: float = 0.20,
        brightness: float = 30.0,
        contrast: float = 0.30,
        p_hsv: float = 0.30,            # v2 new
        hue_delta: float = 10.0,        # v2 new  (OpenCV H is 0-180)
        saturation: float = 0.30,       # v2 new
    ...
    def __call__(self, img: np.ndarray) -> np.ndarray:
        ...
        # 5. Brightness + contrast jitter (always applied)
        img = self._color_jitter(img)

        # 6. HSV hue + saturation jitter  (v2 new)
        if random.random() < self.p_hsv:
    ...
    def _random_rotation(self, img: np.ndarray) -> np.ndarray:
        """Apply random rotation with reflect border fill."""
    ...
    def _random_zoom(self, img: np.ndarray) -> np.ndarray:
        """Scale image by zoom_factor, crop or pad back to original size."""
    ...
    def _color_jitter(self, img: np.ndarray) -> np.ndarray:
        """Per-image brightness and contrast adjustment (BGR space)."""
    ...
    def _hsv_jitter(self, img: np.ndarray) -> np.ndarray:
        """Hue and saturation jitter in HSV colour space.

        Simulates barn/outdoor lighting variation that BGR brightness+contrast
        jitter cannot fully capture (e.g. warm lamp vs. daylight shift).

        OpenCV H channel is in [0, 180], S in [0, 255].
        """
        hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV).astype(np.float32)

        # Hue shift: wrap around [0, 180]
