...
Outputs (created in ./figures/):
    arch_v5_pipeline.pdf   — full forward-pass pipeline (vector)
    arch_v5_pipeline.png   — same, 300 DPI raster
    arch_v5_resblock.pdf   — Pre-Act ResBlock + SE detail (vector)
...
def rblock(ax, cx, cy, w, h, kind, title, sub="", dim="", dim_x_offset=0.45, lw=1.3):
    ...
    if sub:
        ax.text(cx, cy - 0.30 * h, sub,
                ha="center", va="center",
                fontsize=6.5, color=tc, style="italic", zorder=4)
    if dim:
...
def draw_pipeline() -> None:
    ...
    DEFS = [
        ...
        ("input",   "Input Patch",                           "128 × 64  /  BGR",          "",            SH),
        # --- Stage 1 ---
    ...
    ax.text(CX, y_top_fig - 0.82,
            "Pre-Activation ResBlocks  +  SE Attention  +  BNNeck   ≈ 714 k parameters",
            ha="center", va="top",
...
def draw_resblock() -> None:
    ...
    for i in range(len(main_cx) - 1):
        ...
        ax.annotate("",
                    xy=(x_b + 0.02, Y_MAIN), xytext=(x_a - 0.02, Y_MAIN),
                    arrowprops=dict(arrowstyle="-|>", color="#2C3E50",
                                    lw=1.1, mutation_scale=9))
    ...
    ax.text((x_add_right + x_se_left) / 2, Y_MAIN - BH / 2 - 0.12,
            "Add output",
            ha="center", va="top", fontsize=6.5, color="#2C3E50", style="italic")

    # Output arrow + label
    x_out = main_cx[-1] + path[-1][2] / 2 + 0.45
    ax.annotate("",
                xy=(x_out, Y_MAIN),
                xytext=(main_cx[-1] + path[-1][2] / 2 + 0.02, Y_MAIN),
                arrowprops=dict(arrowstyle="-|>", color="#2C3E50",
                                lw=1.1, mutation_scale=9))
    ...
    ax.text(x_in, Y_MAIN - BH / 2 - 0.30,
            ...
            ha="left", va="top", fontsize=7, color="#5D6D7E", style="italic")
    ...
    ax.text((se_box_x0 + se_box_x1) / 2, se_box_y1 - 0.10,
            ...
            ha="center", va="top",
            fontsize=8, fontweight="bold", color=PAL["se"][1])
    ...
    for i in range(len(se_cx) - 1):
        ...
        ax.annotate("",
                    xy=(x_b + 0.02, Y_SE), xytext=(x_a - 0.02, Y_SE),
                    arrowprops=dict(arrowstyle="-|>", color=PAL["se"][1],
                                    lw=1.1, mutation_scale=9))
    ...
    ax.text((x_se_in + x_mult) / 2, fork_y - 0.12,
            ...
            ha="center", va="top", fontsize=6.5, color=PAL["se"][1],
            style="italic")
    ...
    ax.annotate("",
                xy=(x_se_out, Y_SE),
                xytext=(se_cx[-1] + se_path[-1][2] / 2 + 0.02, Y_SE),
                arrowprops=dict(arrowstyle="-|>", color=PAL["se"][1],
                                lw=1.1, mutation_scale=9))
